...

{
  ...
  const handleDownload = (format) => 
  {
    if (format) 
    {
      downloadResume(candidate.resume, format);
    }
    setShowConfirmation(false); // Close the confirmation box after selection
  };
  ...
  const highlightKeyword = (text) => 
  {
   ...
   //This is modified by vaibhavi kawarkhe Date:25-10-2024
      ...
      <div className="card-content">
       ...
       <div className="top-section">
        ...
        <div className="buttons-section">
          ...
          <button className="view-more-btn" onClick={() => setShowConfirmation(true)}>
            <FaRegFileAlt style={{ marginRight: "5px" }} />
            Download Resume
          </button>

          {/* If confirmation is showing, render both overlay and confirmation box */}
          {/* Custom confirmation box */}
          {showConfirmation && (
            <>
            {/* Background overlay */}
            <div className="background-overlay" onClick={() => setShowConfirmation(false)}></div>
            
            <div className="confirmation-box">
              <p>How would you like to download the resume?</p>
              <div className="button-group">
                <button onClick={() => handleDownload('pdf')}>Download as PDF</button>
                <button onClick={() => handleDownload('docx')}>Download as Word</button>
          ...
          <button className="view-more-btn" onClick={() => openResume(candidate.resume)}>
              View Resume
          </button>
